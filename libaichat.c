...
json_decode(const gchar *data, gssize len)
{
	...
	{
		purple_debug_error("aichat", "Error parsing JSON: %s\n", data ? data : "(null)");
	} else {
...
static void
aichat_http_request_cb(PurpleHttpConnection *http_conn, PurpleHttpResponse *response, gpointer user_data)
{
	AiChatApiConnection *conn = user_data;
	const gchar *data;
	gsize len;
	JsonObject *obj;
	...
	if (data == NULL || len == 0) {
		if (conn->error_callback != NULL) {
			...
			purple_debug_error("aichat", "Error parsing response: %s\n", data);
		}
	} else {
...
static AiChatApiConnection *
aichat_provider_http_request(AiChatAccount *cga, const gchar *full_url, const JsonObject *obj, AiChatCallbackFunc callback, gpointer user_data)
{
	PurpleConnection *pc = cga->pc;
	AiChatApiConnection *conn;
	...
	conn = g_new0(AiChatApiConnection, 1);
	conn->cga = cga;
	conn->user_data = user_data;
	conn->callback = callback;
	
	conn->http_conn = purple_http_request(pc, request, aichat_http_request_cb, conn);
...
static AiChatApiConnection *
aichat_http_request(AiChatAccount *cga, const gchar *path, const JsonObject *obj, AiChatCallbackFunc callback, gpointer user_data)
{
	PurpleConnection *pc = cga->pc;
	...
	if (provider == NULL) {
		purple_debug_warning("aichat", "Provider '%s' not found, falling back to OpenAI\n", provider_name);
	...
	url = g_strdup_printf("https://%s%s", AICHAT_API_HOST, path);
...
static void
aichat_create_icon_cb(AiChatAccount *cga, JsonObject *obj, gpointer user_data)
{
	gchar *id = user_data;
...
static void
aichat_create_icon(AiChatAccount *cga, const gchar *id, const gchar *instructions)
{
	...
	aichat_http_request(cga, "/v1/images/generations", obj, aichat_create_icon_cb, g_strdup(id));
}

static void
aichat_create_thread_cb(AiChatAccount *cga, JsonObject *obj, gpointer user_data)
{
	...
	AiChatBuddy *cbuddy = purple_buddy_get_protocol_data(buddy);
...
static void
aichat_create_assistant_cb(AiChatAccount *cga, JsonObject *obj, gpointer user_data)
{
	const gchar *id = json_object_get_string_member(obj, "id");
	const gchar *name = json_object_get_string_member(obj, "name");

	if (id == NULL || id[0] == 0) {
		purple_debug_error("aichat", "Error creating assistant\n");
		return;
	}
	...
	if (TRUE) {
		...
		aichat_create_icon(cga, id, instructions);
	}

	PurpleBuddy *buddy = purple_find_buddy(cga->account, id);
	AiChatBuddy *cbuddy = g_new0(AiChatBuddy, 1);
	...
	aichat_http_request(cga, "/v1/threads", thread_obj, aichat_create_thread_cb, g_strdup(id));
	json_object_unref(thread_obj);
}

static void
aichat_create_assistant(AiChatAccount *cga, const gchar *instructions)
{
	JsonObject *obj = json_object_new();
	...
	aichat_http_request(cga, "/v1/assistants", obj, aichat_create_assistant_cb, cga);
	json_object_unref(obj);
}

static void
aichat_send_message_cb(AiChatAccount *cga, JsonObject *obj, gpointer user_data)
{
	gchar *assistant_id = user_data;
	JsonArray *data_arr = json_object_get_array_member(obj, "data");
	JsonObject *data_obj = json_array_get_object_element(data_arr, 0);
	...
	if (text_value == NULL) {
		...
		purple_debug_error("aichat", "No content found in message\n");
		g_free(assistant_id);
		return;
	}
...
static void
aichat_send_run_cb(AiChatAccount *cga, JsonObject *obj, gpointer user_data)
{
	gchar *assistant_id = user_data;
	...
	if (purple_strequal(status, "completed")) {
		...
		aichat_http_request(cga, url, NULL, aichat_send_message_cb, g_strdup(assistant_id));
		g_free(url);
	} else if (status != NULL) {
		// wait for the run to complete
		purple_debug_info("aichat", "Run not completed yet\n");
		gchar *url = g_strdup_printf("/v1/threads/%s/runs/%s", thread_id, run_id);
		aichat_http_request(cga, url, NULL, aichat_send_run_cb, g_strdup(assistant_id));
		g_free(url);
	}
...
static void
aichat_send_message(AiChatAccount *cga, const gchar *id, const gchar *message)
{
	PurpleBuddy *buddy = purple_find_buddy(cga->account, id);
	if (buddy == NULL) {
		purple_debug_error("aichat", "Buddy not found: %s\n", id);
		return;
	}

	const gchar *thread_id = purple_blist_node_get_string(PURPLE_BLIST_NODE(buddy), "thread_id");
	if (thread_id == NULL || thread_id[0] == 0) {
		purple_debug_error("aichat", "Thread ID not found for buddy: %s\n", id);
		return;
	}
	...
	aichat_http_request(cga, url, obj, NULL, NULL);
	...
	aichat_http_request(cga, url, obj, aichat_send_run_cb, g_strdup(id));
...
static void
aichat_chat_completion_cb(AiChatAccount *cga, JsonObject *obj, gpointer user_data)
{
	...
	provider = llm_provider_get(cga->provider_type);
	if (provider == NULL) {
		purple_debug_error("aichat", "No provider found for type %d\n", cga->provider_type);
		g_free(buddy_id);
		return;
	}
	...
	if (provider->validate_response && !provider->validate_response(obj, &error)) {
		purple_debug_error("aichat", "Invalid response: %s\n", error ? error->message : "Unknown error");
		if (error) {
	...
	if (provider->parse_response) {
		...
		if (response_text == NULL) {
			purple_debug_error("aichat", "Failed to parse response: %s\n", error ? error->message : "Unknown error");
			if (error) {
				purple_serv_got_im(cga->pc, buddy_id, error->message, PURPLE_MESSAGE_ERROR | PURPLE_MESSAGE_RECV, time(NULL));
				g_error_free(error);
			}
	...
	if (buddy) {
		AiChatBuddy *cgb = purple_buddy_get_protocol_data(buddy);
		if (cgb) {
			AiChatHistory *hist = g_new0(AiChatHistory, 1);
...
static void
aichat_create_simple_bot(AiChatAccount *cga, const gchar *instructions)
{
	...
	/* Notify user */
	purple_conversation_write(purple_find_conversation_with_account(PURPLE_CONV_TYPE_IM, AICHAT_INSTRUCTOR_ID, cga->account),
		AICHAT_INSTRUCTOR_ID,
...
static void
aichat_send_chat_message(AiChatAccount *cga, const gchar *buddy_id, const gchar *message)
{
	PurpleBuddy *buddy;
	AiChatBuddy *cgb;
	LLMProvider *provider;
	...
	if (cgb == NULL) {
		purple_debug_error("aichat", "No protocol data for buddy: %s\n", buddy_id);
		return;
	}
	...
	if (provider->format_request) {
		...
		if (request == NULL) {
			purple_debug_error("aichat", "Failed to format request\n");
			return;
		}
	} else {
		purple_debug_error("aichat", "Provider has no format_request function\n");
		return;
	}
	...
	aichat_provider_http_request(cga, url, request, aichat_chat_completion_cb, g_strdup(buddy_id));
...
static void
aichat_fetch_assistants_cb(AiChatAccount *cga, JsonObject *obj, gpointer user_data)
{
	JsonArray *data_arr = json_object_get_array_member(obj, "data");
...
static void
aichat_fetch_assistants(AiChatAccount *cga)
{
	aichat_http_request(cga, "/v1/assistants", NULL, aichat_fetch_assistants_cb, NULL);
}
...
static const char *
aichat_list_icon(PurpleAccount *account, PurpleBuddy *buddy)
{
	return "aichat";
}

void
aichat_tooltip_text(PurpleBuddy *buddy, PurpleNotifyUserInfo *user_info, gboolean full)
{
	AiChatBuddy *cbuddy = purple_buddy_get_protocol_data(buddy);
...
static void
aichat_buddy_free(PurpleBuddy *buddy)
{
	AiChatBuddy *cbuddy = purple_buddy_get_protocol_data(buddy);
	if (cbuddy) {
...
GList *
aichat_status_types(PurpleAccount *account)
{
	GList *types = NULL;
...
gint
aichat_send_im(PurpleConnection *gc, const char *who, const char *message, PurpleMessageFlags flags)
{
	AiChatAccount *cga = purple_connection_get_protocol_data(gc);
	
	/* Check if we're using OpenAI's assistants API or generic chat */
	if (cga->provider_type == LLM_PROVIDER_OPENAI) {
		/* Use OpenAI's assistants API */
		if (purple_strequal(who, AICHAT_INSTRUCTOR_ID)) {
			// Treat this as creating a new assistant
			aichat_create_assistant(cga, message);	
		} else {
			aichat_send_message(cga, who, message);
		}
	} else {
		/* Use generic chat completion API */
		if (purple_strequal(who, AICHAT_INSTRUCTOR_ID)) {
			/* For non-OpenAI providers, create a simple bot */
			aichat_create_simple_bot(cga, message);
		} else {
			aichat_send_chat_message(cga, who, message);
		}
	}
...
void
aichat_fake_group_buddy(PurpleConnection *pc, const char *who, const char *old_group, const char *new_group)
{
	// Do nothing to stop the remove+add behaviour
}
void
aichat_fake_group_rename(PurpleConnection *pc, const char *old_name, PurpleGroup *group, GList *moved_buddies)
{
	// Do nothing to stop the remove+add behaviour
}

static void
aichat_alias_buddy(PurpleConnection *pc, const char *who, const char *alias)
{
	AiChatAccount *cga = purple_connection_get_protocol_data(pc);
...
static void
aichat_login(PurpleAccount *account)
{
	PurpleConnection *pc = purple_account_get_connection(account);
	AiChatAccount *cga = g_new0(AiChatAccount, 1);
	...
	if (cga->provider_type >= LLM_PROVIDER_COUNT) {
		purple_debug_warning("aichat", "Invalid provider '%s', defaulting to OpenAI\n", provider_name);
		cga->provider_type = LLM_PROVIDER_OPENAI;
	}
	...
	if (!purple_find_buddy(account, AICHAT_INSTRUCTOR_ID)) {
		purple_blist_add_buddy(purple_buddy_new(account, AICHAT_INSTRUCTOR_ID, NULL), NULL, NULL, NULL);
	}
	...
	if (api_key == NULL || api_key[0] == 0) {
		...
		if (cga->provider_type == LLM_PROVIDER_OPENAI) {
			purple_notify_uri(pc, AICHAT_API_KEY_URL);
		}
	} else {
		purple_prpl_got_user_status(account, AICHAT_INSTRUCTOR_ID, "available", NULL);
		
		if (cga->provider_type == LLM_PROVIDER_OPENAI) {
			purple_serv_got_im(pc, AICHAT_INSTRUCTOR_ID, 
			...
			gchar *welcome_msg = g_strdup_printf(
				...
				provider_name);
			purple_serv_got_im(pc, AICHAT_INSTRUCTOR_ID, welcome_msg, 
				PURPLE_MESSAGE_SYSTEM | PURPLE_MESSAGE_RECV, time(NULL));
			g_free(welcome_msg);
...
static void
aichat_close(PurpleConnection *pc)
{
	AiChatAccount *sa;
	GSList *buddies;
...
gboolean
aichat_offline_message(const PurpleBuddy *buddy)
{
	return TRUE;
}

static PurpleCmdRet
aichat_cmd_model(PurpleConversation *conv, const gchar *cmd, gchar **args, gchar **error, void *data)
{
	// update the model for the assistant
	const gchar *name = purple_conversation_get_name(conv);
	if (name == NULL || name[0] == 0 || purple_strequal(name, AICHAT_INSTRUCTOR_ID)) {
		return PURPLE_CMD_RET_FAILED;
	}

	AiChatAccount *cga = purple_connection_get_protocol_data(purple_conversation_get_connection(conv));
...
#if PURPLE_VERSION_CHECK(3, 0, 0)
	typedef struct _AiChatProtocol
	{
		PurpleProtocol parent;
	} AiChatProtocol;

	typedef struct _AiChatProtocolClass
	{
		PurpleProtocolClass parent_class;
	} AiChatProtocolClass;

	G_MODULE_EXPORT GType aichat_protocol_get_type(void);
	#define aichat_TYPE_PROTOCOL             (aichat_protocol_get_type())
	#define aichat_PROTOCOL(obj)             (G_TYPE_CHECK_INSTANCE_CAST((obj), aichat_TYPE_PROTOCOL, AiChatProtocol))
	#define aichat_PROTOCOL_CLASS(klass)     (G_TYPE_CHECK_CLASS_CAST((klass), aichat_TYPE_PROTOCOL, AiChatProtocolClass))
	#define aichat_IS_PROTOCOL(obj)          (G_TYPE_CHECK_INSTANCE_TYPE((obj), aichat_TYPE_PROTOCOL))
	#define aichat_IS_PROTOCOL_CLASS(klass)  (G_TYPE_CHECK_CLASS_TYPE((klass), aichat_TYPE_PROTOCOL))
	#define aichat_PROTOCOL_GET_CLASS(obj)   (G_TYPE_INSTANCE_GET_CLASS((obj), aichat_TYPE_PROTOCOL, AiChatProtocolClass))

	static PurpleProtocol *aichat_protocol;
#else
...
)
{
	...
	purple_cmd_register("model", "s", PURPLE_CMD_P_PLUGIN, PURPLE_CMD_FLAG_IM |
						...
						AICHAT_PLUGIN_ID, aichat_cmd_model,
...
)
{
	...
	// act = purple_protocol_action_new(_("Search for Teams Contacts"), aichat_search_users);
...
#if PURPLE_VERSION_CHECK(3, 0, 0)
static void 
aichat_protocol_init(PurpleProtocol *prpl_info) 
{
...
static void 
aichat_protocol_class_init(PurpleProtocolClass *prpl_info) 
{
#endif
	//PurpleProtocolClass
	prpl_info->login = aichat_login;
	prpl_info->close = aichat_close;
	prpl_info->status_types = aichat_status_types;
	prpl_info->list_icon = aichat_list_icon;
#if PURPLE_VERSION_CHECK(3, 0, 0)
}

static void 
aichat_protocol_client_iface_init(PurpleProtocolClientIface *prpl_info) 
{
...
#if !PURPLE_VERSION_CHECK(3, 0, 0)
	info->actions = aichat_actions;
#else
	info->get_actions = aichat_actions;
#endif
	prpl_info->tooltip_text = aichat_tooltip_text;
	prpl_info->normalize = purple_normalize_nocase;
	prpl_info->offline_message = aichat_offline_message;
	prpl_info->get_account_text_table = NULL; // aichat_get_account_text_table;
#if PURPLE_VERSION_CHECK(3, 0, 0)
}

static void 
aichat_protocol_server_iface_init(PurpleProtocolServerIface *prpl_info) 
{
#endif
	
	//PurpleProtocolServerIface
	// prpl_info->get_info = aichat_get_info;
	// prpl_info->set_status = aichat_set_status;
	prpl_info->group_buddy = aichat_fake_group_buddy;
	prpl_info->rename_group = aichat_fake_group_rename;
	prpl_info->alias_buddy = aichat_alias_buddy;
#if PURPLE_VERSION_CHECK(3, 0, 0)
}

static void 
aichat_protocol_im_iface_init(PurpleProtocolIMIface *prpl_info) 
{
#endif
	
	//PurpleProtocolIMIface
#if !PURPLE_VERSION_CHECK(3, 0, 0)
	prpl_info->send_im = aichat_send_im;
#else
	prpl_info->send = aichat_send_im;
#endif
	// prpl_info->send_typing = aichat_send_typing;
#if PURPLE_VERSION_CHECK(3, 0, 0)
}

static void 
aichat_protocol_xfer_iface_init(PurpleProtocolXferInterface *prpl_info) 
{
#endif
	
	//PurpleProtocolXferInterface
	// prpl_info->new_xfer = aichat_new_xfer;
	// prpl_info->send_file = aichat_send_file;
#if !PURPLE_VERSION_CHECK(3, 0, 0)
	// prpl_info->can_receive_file = aichat_can_receive_file;
#else
	// prpl_info->can_receive = aichat_can_receive_file;
#endif
	#if PURPLE_VERSION_CHECK(2, 14, 0)
		// prpl_info->chat_send_file = aichat_chat_send_file;
		// prpl_info->chat_can_receive_file = aichat_chat_can_receive_file;
	#else
		// prpl_info_ext->chat_send_file = aichat_chat_send_file;
		// prpl_info_ext->chat_can_receive_file = aichat_chat_can_receive_file;
	#endif
...
static void 
aichat_protocol_roomlist_iface_init(PurpleProtocolRoomlistIface *prpl_info) 
{
#endif
	
	//PurpleProtocolRoomlistIface
#if !PURPLE_VERSION_CHECK(3, 0, 0)
	// prpl_info->roomlist_get_list = aichat_roomlist_get_list;
#else
	// prpl_info->get_list = aichat_roomlist_get_list;
#endif
	...
	info->version = AICHAT_PLUGIN_VERSION;
	...
	info->homepage = "https://github.com/steven-aranaga/pidgin-aichat-clone";
	...
	// Protocol info
		...
		// prpl_info->add_buddy_with_invite = aichat_add_buddy_with_invite;
	#endif
...
PURPLE_DEFINE_TYPE_EXTENDED(
	AiChatProtocol, aichat_protocol, PURPLE_TYPE_PROTOCOL, 0,

	PURPLE_IMPLEMENT_INTERFACE_STATIC(PURPLE_TYPE_PROTOCOL_CLIENT_IFACE,
	                                  aichat_protocol_client_iface_init)

	PURPLE_IMPLEMENT_INTERFACE_STATIC(PURPLE_TYPE_PROTOCOL_SERVER_IFACE,
	                                  aichat_protocol_server_iface_init)

	PURPLE_IMPLEMENT_INTERFACE_STATIC(PURPLE_TYPE_PROTOCOL_IM_IFACE,
	                                  aichat_protocol_im_iface_init)

	PURPLE_IMPLEMENT_INTERFACE_STATIC(PURPLE_TYPE_PROTOCOL_CHAT_IFACE,
	                                  aichat_protocol_chat_iface_init)

	PURPLE_IMPLEMENT_INTERFACE_STATIC(PURPLE_TYPE_PROTOCOL_PRIVACY_IFACE,
	                                  aichat_protocol_privacy_iface_init)

	PURPLE_IMPLEMENT_INTERFACE_STATIC(PURPLE_TYPE_PROTOCOL_ROOMLIST_IFACE,
	                                  aichat_protocol_roomlist_iface_init)

	PURPLE_IMPLEMENT_INTERFACE_STATIC(PURPLE_TYPE_PROTOCOL_XFER,
	                                  aichat_protocol_xfer_iface_init)
);

static gboolean
libpurple3_plugin_load(PurplePlugin *plugin, GError **error)
{
	aichat_protocol_register_type(plugin);
	aichat_protocol = purple_protocols_add(PURPLE_TYPE_PROTOCOL, error);
	if (!aichat_protocol)
		return FALSE;
...
libpurple3_plugin_unload(PurplePlugin *plugin, GError **error)
{
	...
	if (!purple_protocols_remove(aichat_protocol, error))
		return FALSE;
...
plugin_query(GError **error)
{
	return purple_plugin_info_new(
		...
		"version",      aichat_PLUGIN_VERSION,
		"category",     N_("Protocol"),
		"summary",      N_("AI Chat Protocol Plugin"),
		"description",  N_("AI Chat Protocol Plugin"),
		"website",      "https://github.com/steven-aranaga/pidgin-aichat-clone",
...
PURPLE_PLUGIN_INIT(aichat, plugin_query, libpurple3_plugin_load, libpurple3_plugin_unload);
#else
	
static PurplePluginInfo aLovelyBunchOfCoconuts;
PURPLE_INIT_PLUGIN(aichat, plugin_init, aLovelyBunchOfCoconuts);
#endif
