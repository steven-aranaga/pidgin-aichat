...
install-icons: icons/16/chatgpt.png icons/22/chatgpt.png icons/48/chatgpt.png
	mkdir -m $(DIR_PERM) -p $(PLUGIN_ICONS_DEST)/16
	mkdir -m $(DIR_PERM) -p $(PLUGIN_ICONS_DEST)/22
	mkdir -m $(DIR_PERM) -p $(PLUGIN_ICONS_DEST)/48
	install -m $(FILE_PERM) -p icons/16/chatgpt.png $(PLUGIN_ICONS_DEST)/16/chatgpt.png
	install -m $(FILE_PERM) -p icons/22/chatgpt.png $(PLUGIN_ICONS_DEST)/22/chatgpt.png
	install -m $(FILE_PERM) -p icons/48/chatgpt.png $(PLUGIN_ICONS_DEST)/48/chatgpt.png

installer: purple-chatgpt.nsi libchatgpt.dll
	$(MAKENSIS) "/DPIDGIN_VARIANT"="Pidgin" "/DPRODUCT_NAME"="purple-chatgpt" "/DINSTALLER_NAME"="purple-chatgpt-installer" "/DJSON_GLIB_DLL"="libjson-glib-1.0.dll" purple-chatgpt.nsi

translations: po/purple-chatgpt.pot

po/purple-chatgpt.pot: $(PURPLE_C_FILES)
	$(XGETTEXT) $^ -k_ --no-location -o $@

po/%.po: po/purple-chatgpt.pot
	msgmerge $@ po/purple-chatgpt.pot > tmp-$*
	mv -f tmp-$* $@
...
%-locale-install: po/%.mo
	install -D -m $(FILE_PERM) -p po/$(*F).mo $(LOCALEDIR)/$(*F)/LC_MESSAGES/purple-chatgpt.mo
